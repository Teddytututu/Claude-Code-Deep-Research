...
#### 综述论文优先搜索
```
适用条件: 新兴研究领域、需要建立知识图谱时
执行方法: 使用 'survey OR review OR tutorial' 作为必选搜索词
预期效果: 快速建立领域知识框架，找到高质量综述
效果: high
发现时间: 2026-02-21
```
...
#### 并行搜索组合策略
```
适用条件: 交叉领域主题、需要覆盖多个研究维度时
执行方法: 设计多个互补的搜索查询，每个查询聚焦不同关键词组合
预期效果: 候选论文数量增加 2-3 倍
效果: high
发现时间: 2026-02-21
```
...
#### 工具容错处理
```
适用条件: 使用 MCP 工具时（如 mcp__zread__*）
执行方法: 先用 git ls-remote 验证仓库存在性，再用专用工具分析
预期效果: 避免因目标不存在导致的工具错误（且不消耗 WebSearch 额度）
效果: medium
发现时间: 2026-02-21
```
