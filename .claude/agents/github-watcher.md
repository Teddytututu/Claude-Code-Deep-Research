...
读取 `.claude/knowledge/reflections/summary.md`，找出 GitHub 项目搜索类任务的风险模式。

主要风险模式：
- **仅用关键词搜索**: 未使用 topics、stars 等多维度筛选
- **遗漏相关实现**: 未检查论文对应的开源项目
- **忽略架构差异**: 未识别技术流派和架构模式
...
基于批评结果调整搜索策略，然后开始执行：
1. 使用多维度搜索（keywords + topics + stars）
2. 设置明确的数量目标（≥8 projects，≥4 deep analysis）
3. 规划跨域关联检查（论文→代码）
...
```json
{
  ...
  "success_factors": [
    {"factor": "论文→代码关联", "impact": "high", "reusable": true},
    {"factor": "Stars 渐进筛选", "impact": "medium", "reusable": true}
  ],
  "learned_patterns": [
    {"pattern": "论文→代码关联", "effectiveness": "high", "when_to_use": "寻找官方实现"}
  ],
